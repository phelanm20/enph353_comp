...
img_dir = "/home/maria/enph353_ws/src/competition/scripts/images/plate_detection_training/"
...
#pics = np.empty(len(env_img_paths), dtype=object)
#for n in range(0, len(env_img_paths)):
#    pics[n] = cv.imread(env_img_paths[n], 0)[np.newaxis] #Read as grayscale
...
for n in range(0, len(listdir(img_dir))):
    labels[n] = int(listdir(img_dir)[n][-5])
...
conv_model.add(layers.Dense(5, activation='softmax')) ## of labels
conv_model.summary()
